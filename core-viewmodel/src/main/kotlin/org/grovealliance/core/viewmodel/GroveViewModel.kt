...
/**
 * Retrieves a [ViewModel] of type [VM] from the Grove dependency graph, scoped to the given
 ...
 */
@Composable
inline fun <reified VM : ViewModel> groveViewModel(
...
/**
 * Returns a [Lazy] delegate to access a [ViewModel] of type [VM] registered in the Grove
 * dependency graph, scoped to this [Fragment]'s own [androidx.lifecycle.ViewModelStore].
 *
 * Mirrors [androidx.fragment.app.viewModels] but uses the Grove factory under the hood.
 *
 * Usage:
 * ```kotlin
 * class HomeFragment : Fragment() {
 *     private val viewModel: HomeViewModel by groveViewModels()
 * }
 * ```
 *
 * @param VM The ViewModel type to retrieve. Must have been registered via [viewModels].
 ...
 *   [CreationExtras.Empty] otherwise.
 */
inline fun <reified VM : ViewModel> Fragment.groveViewModels(
    noinline ownerProducer: () -> ViewModelStoreOwner = { this },
    noinline extrasProducer: (() -> CreationExtras)? = null,
): Lazy<VM> = ViewModelLazy(
    viewModelClass = VM::class,
    storeProducer = { ownerProducer().viewModelStore },
    factoryProducer = { groveViewModelFactory() },
 ...
 *
 * Mirrors [androidx.fragment.app.activityViewModels] but uses the Grove factory under the hood.
 *
 * Usage:
 * ```kotlin
 * class HomeFragment : Fragment() {
 *     private val sharedViewModel: SharedViewModel by groveActivityViewModels()
 * }
 * ```
 *
 * @param VM The ViewModel type to retrieve. Must have been registered via [viewModels].
 ...
 */
inline fun <reified VM : ViewModel> Fragment.groveActivityViewModels(
    noinline extrasProducer: (() -> CreationExtras)? = null,
): Lazy<VM> = groveViewModels(
 ...
 *
 * Mirrors [androidx.activity.viewModels] but uses the Grove factory under the hood.
 *
 * Usage:
 * ```kotlin
 * class MainActivity : ComponentActivity() {
 *     private val viewModel: MainViewModel by groveViewModels()
 * }
 * ```
 *
 * @param VM The ViewModel type to retrieve. Must have been registered via [viewModels].
 ...
 */
inline fun <reified VM : ViewModel> ComponentActivity.groveViewModels(
    noinline extrasProducer: (() -> CreationExtras)? = null,
): Lazy<VM> = ViewModelLazy(
    viewModelClass = VM::class,
...
/**
 * Returns a [ViewModelProvider.Factory] backed by the Grove dependency graph, for use in
 * non-composable contexts such as [Fragment] or [ComponentActivity].
 */
@PublishedApi
internal fun groveViewModelFactory(): ViewModelProvider.Factory {
    val viewModelFactories = requireDependency<ViewModelFactories>()
